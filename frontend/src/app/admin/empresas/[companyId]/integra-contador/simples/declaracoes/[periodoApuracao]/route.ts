...
// Proxy pro CONSDECLARACAO13 já cadastrado no backend (cacheado lá) — dá
// pra conferir se uma competência já foi declarada antes de tentar
// transmitir de novo, sem gastar uma chamada de Declarar.
export async function GET(
  ...
  try {
    response = await fetch(
      `${process.env.INTEGRA_CONTADOR_URL}/contribuintes/${company.cnpj}/simples/declaracoes/${periodoApuracao}`,
      {
    ...
    return NextResponse.json(
      { error: "Não foi possível consultar as declarações agora. Tente novamente em instantes." },
      { status: 502 },
    );
  }
