...
import { STATUS_PILL_CLASSES, statusDocumento } from "@/app/legalizacao/status";
...
export default async function LegalizacaoEmpresaPage(
  ...
  const tiposAplicaveis = (tipos ?? []).filter((t) => !tiposNaoAplicaveis.has(t.id));
  const tiposNaoAplicaveisNomes = (tipos ?? [])
    .filter((t) => tiposNaoAplicaveis.has(t.id))
    .map((t) => t.nome);
